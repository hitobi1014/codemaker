...
public class TeacherLessonController {
	...
	public List<LessonVO> selectLesson(Model model,String subId,HttpSession session) {
		...
		try {
			...
			lessonList = lessonService.selectLesson(lessonVO);
...
//		lessonVO.setSubId(subId);
	...
	public String updateViewLessonIndex(String lidxId, Model model,String lesId,LessonIndexVO lesIdxVO,String lidxCont, int lidxNum) {
		List<LessonIndexVO> lesIdxList = new ArrayList<>();
		int delCnt = 0;
		int upCnt = 0;
		logger.debug("넘어온 값:{},{}", lidxId, lesId);
